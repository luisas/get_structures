...
class RowChecker:
    ...
    def validate_and_transform(self, row):
        ...
        self._validate_db(row)
        self._seen.add((row[self._id_col], row[self._db_col]))
    ...
    def _validate_db(self, row):
        """Assert that the fasta entry is non-empty and has the right format."""
        if len(row[self._db_col]) <= 0:
            raise AssertionError("At least the first FASTQ file is required.")
        row[self._db_col] = row[self._db_col].replace(" ", "_")
